...
const LogFilter = {
  provideBackgroundTips() {
    return {
      packageName: "log-filter",
      tips: [
        "{% if keys['log-filter:toggle'] %}You can fold away every line of a log that does not match your filter with {{ 'log-filter:toggle' | keystroke }}{% else %}You can fold away every line of a log that does not match your filter.{% endif %}",
      ],
    };
  },
  ...
  activate(state) {
    ...
    this.disposables.add(
      lumine.workspace.observeActivePaneItem((item) => this.observeItem(item)),
      lumine.commands.add("lumine-workspace", {
        "log-filter:toggle": () => this.toggle(),
        "log-filter:toggle-focus": () => this.toggleFocus(),
      }),
      lumine.config.onDidChange("log-filter.autoShow", () => {
        this.observeItem(lumine.workspace.getActivePaneItem());
      }),
      lumine.config.onDidChange("log-filter.grammarScopes", () => {
        this.observeItem(lumine.workspace.getActivePaneItem());
      }),
    );
  },
  ...
  observeItem(item) {
    ...
    this.grammarSubscription = item.observeGrammar(() => {
      if (this.isLogEditor(item) && lumine.config.get("log-filter.autoShow")) {
        this.showPanel(item);
      } else {
        this.hidePanel();
      }
  ...
  showPanel(editor) {
    ...
    if (this.panel == null) {
      this.panel = lumine.workspace.addBottomPanel({
